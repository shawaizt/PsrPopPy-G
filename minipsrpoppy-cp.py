...
c = 299792458 #m/s
k = (8 * np.pi**2 * (12*1000*100)**6) / (3 * (1.7*10**45) * (c)**3) #spindown model constant. this is with R=12km, I=1.7*10^45 g cm^2, from Gonthier+'18
...
surveys_path = '/home/user/Gamma-PsrPopPy/surveys/cp'
...
#%%
#Load the evolved list of CPs:
def read_large_file(file_path, custom_column_names):
    #specify the column indices to read
    columns_to_read = [0, 1, 2, 3, 4]

    #read the file into a numpy array, only extracting the specified columns
    data = np.genfromtxt(file_path, delimiter=None, usecols=columns_to_read, dtype=float, autostrip=True, missing_values=['NaN', 'NA', 'N/A'], filling_values=np.nan)

    # Check if the number of columns matches the specified columns
    if data.shape[1] != len(columns_to_read):
        raise ValueError("Number of specified columns doesn't match the number of columns in the file.")

    #remove rows with NaN values
    data = data[~np.isnan(data).any(axis=1)]

    return data, [custom_column_names[i] for i in columns_to_read]

"""
Set path which contains the a file with list of CPs evoled to a time t=age which contains their age, period at time t, 
period derivative, magnetic inclincation and optionally their braking index. It is recommended that it contains at least 
2 million pulsars.

You can modify this based on how you do the evolve process and store your output from that.
"""
file_path = '/home/user/multi-synthesis/evolved-cps.txt'
custom_column_names = ['Age', 'p0', 'p1', 'alpha', 'n']

data_subset, column_names_subset = read_large_file(file_path, custom_column_names)

# Access the values in the first four columns with custom column names
column_values = {}

for i, column_name in enumerate(column_names_subset):
    column_values[column_name] = data_subset[:, i]
    
t_to_s = 365.25*864000
r = 12e5 #cm
I = 1.7e45 #g cm^2

age_values = column_values['Age']*10**6 #in yrs
pt_values = column_values['p0'] #in seconds
pdot_values = column_values['p1']
alpha_values = column_values['alpha'] #in radians
alpha_vals = column_values['alpha']

alpha0_values = alpha_vals * np.exp(age_values/1.0e7)

a_0 = 8*np.pi**2 * r**6 / (3*c*c*c*I)
b_values = np.sqrt(pdot_values * pt_values / (a_0 * np.sin(alpha0_values)**2))
...
surveylist = ['PMSURV_ST_EDITS', 'SWINMB1', 'SWINMB2']
...
max_age = 1.0e8 #yrs
z_scale = 0.05 #kpc
bv_stdev = 180.0 #birth velocity sigma to be used to draw from a gaussian with mean of 0

ndet = 0
    
while ndet < 1323 and i < len(age_values):
    
    age_p = age_values[i]
    pt = pt_values[i]
    
    if pt > 10.0 or pt < 0.0:
        i += 1
        continue
    ...
    #Find luminosity
    l1400 = pyfunc.luminosity_fk06(pt, pdot, alpha=-1.5, beta=0.5, gamma=0.18) 
